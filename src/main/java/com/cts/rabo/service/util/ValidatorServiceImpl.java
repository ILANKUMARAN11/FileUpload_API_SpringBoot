...
public class ValidatorServiceImpl extends VoMapper implements ValidatorService {
	...
	public List<Statements> extractDupicateRef(List<StatementRecords> statemnets) {
		...
		return statemnets.stream().collect(Collectors.groupingBy(StatementRecords::getRefrence)).entrySet().stream()
				.filter(e -> e.getValue().size() > 1).flatMap(e -> e.getValue().stream()).map(d -> entityToVo.apply(d))
	...
	public List<Statements> validateEndBalance(List<StatementRecords> statemnets) {
		...
		Predicate<StatementRecords> p = new Predicate<StatementRecords>() {
			...
			public boolean test(StatementRecords e) {
				...
				LOGGER.info("Start balance ::" + e.getStartBalance());
				LOGGER.info("Mutation balance ::" + e.getMutation());
				double spent = Double.parseDouble(df.format(e.getStartBalance() + e.getMutation()));
				LOGGER.info("SUM of start balance and mutation balance ::" + spent);
				LOGGER.info("End balance ::" + e.getEndBalance());
		...
		return statemnets.stream().filter(p).map(d -> entityToVo.apply(d)).collect(Collectors.toList());
